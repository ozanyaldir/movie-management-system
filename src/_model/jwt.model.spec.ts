...
describe('newJWTPayload', () => {
  ...
  it('maps fields correctly', () => {
    const dto = newJWTPayload('user-guid-1', UserType.Customer, 'session-123');
  ...
  it('preserves enum value for Manager', () => {
    const dto = newJWTPayload('admin-guid-1', UserType.Manager, 'session-abc');

    expect(dto.user_type).toBe(UserType.Manager);
  });

  it('returns a plain immutable-like object structure', () => {
    const dto = newJWTPayload('user-guid-2', UserType.Customer, 's-999');
